...
export function clientEmailHtml(content: SiteContent, lead: Lead) {
  ...
  const body = paragraphs(emails.clientBody.replaceAll("{{name}}", lead.name));
  const closing = paragraphs(emails.clientClosing);
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-top:6px solid #cf3028">
    <div style="padding:28px 30px">
      ...
      <div style="font-size:13px;color:#777;line-height:1.7">
        ...
        ${contact.address ? `<div>${escape(contact.address)}</div>` : ""}
        ${contact.hours ? `<div>${escape(contact.hours)}</div>` : ""}
      </div>
    </div>
  </div>
</div>`;
...
export async function sendClientEmail(lead: Lead, content: SiteContent) {
